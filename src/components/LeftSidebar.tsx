...
function TreeItem({ diagram, diagrams, activeId, onOpen, onDelete, rootId }: {
  diagram: EditorDiagram; diagrams: EditorDiagram[]; activeId: string; rootId: string;
...
export function LeftSidebar({ diagrams, activeId, rootId, onOpen, onAdd, onDelete }: {
  diagrams: EditorDiagram[]; activeId: string; rootId: string;
  ...
  return <aside className="project-tree" aria-label="Дерево диаграмм">
    ...
    <div className="tree-hint">В IDEF0 двойное нажатие на блок открывает декомпозицию.</div>
  </aside>
}
